//
//  WifiSelectDeliveryAddressViewController.m
//  Flatland
//
...
#import "WifiSelectDeliveryAddressViewController.h"
#import "WaitIndicator.h"
#import "User.h"
#import "AddressCell.h"
#import "AlertView.h"

@interface WifiSelectDeliveryAddressViewController () <UITableViewDataSource, UITableViewDelegate>
...
@implementation WifiSelectDeliveryAddressViewController
...
- (IBAction)performSequeIfValid {
    ...
    for(Address *a in self.addresses) {
        if([a.ID isEqualToString:[User activeUser].preferredDeliveryAddressID]) {
            a.preferred = YES;
            address = a;
            break;
        }
    ...
    [[User activeUser] updateDeliveryAddress:address completion:^(BOOL success) {
        if(success) {
            [self performSegueWithIdentifier:@"configureBillingAddress" sender:self];
        } else {
